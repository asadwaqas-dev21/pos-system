        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.Text('Super POS', style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold)),
              pw.Text('123 Main Street, City'),
              pw.Text('Tel: +1 234 567 890'),
              pw.Text('Receipt: ${order.orderId}', style: const pw.TextStyle(fontSize: 12)),
              if (order.customerName != null && order.customerName!.isNotEmpty)
                pw.Text('Customer: ${order.customerName}', style: const pw.TextStyle(fontSize: 12)),
              pw.Text('Date: ${DateFormat('dd-MM-yyyy HH:mm:ss').format(order.date)}', style: const pw.TextStyle(fontSize: 12)),
              ...
              ...order.items.map((item) {
                return pw.Padding(
                  ...
                  child: pw.Row(
                    mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                    children: [
                      pw.Expanded(flex: 3, child: pw.Text(item.product.name, maxLines: 2)),
                      pw.Expanded(flex: 1, child: pw.Text('${item.quantity}', textAlign: pw.TextAlign.center)),
                      pw.Expanded(flex: 2, child: pw.Text('PKR ${item.totalPrice.toStringAsFixed(2)}', textAlign: pw.TextAlign.right)),
                    ]
                  )
                );
              }).toList(),
              ...
              pw.Row(
                ...
                children: [
                  ...
                  pw.Text('PKR ${order.total.toStringAsFixed(2)}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 16))
                ]
              ),
              
              pw.Divider(thickness: 1, height: 20),
              pw.Text('Thank you for shopping with us!', textAlign: pw.TextAlign.center),
              pw.BarcodeWidget(
                data: order.orderId,
